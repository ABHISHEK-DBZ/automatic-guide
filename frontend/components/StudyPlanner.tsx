...
export default function StudyPlanner() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="gc-card p-6 border-b-2 border-cyan-500/20">
            ...
            <div className="flex bg-slate-800/50 rounded-lg p-1">
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-semibold transition-all ${
                  viewMode === 'list'
                    ? 'bg-gradient-to-r from-cyan-500 to-purple-500 text-white shadow-[0_0_10px_rgba(0,212,255,0.3)]'
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="gc-card p-4 bg-gradient-to-br from-cyan-500/10 to-transparent border-cyan-500/30">
            <div className="flex items-center gap-3">
      ...
      {/* Add Plan Form */}
      {showAdd && (
        <div className="gc-card p-6 border-2 border-cyan-500/30 space-y-4">
          ...
          <input 
            ...
            className="w-full px-4 py-3 bg-[rgb(var(--bg-elevated))] border border-white/10 rounded-lg text-white placeholder-slate-500 focus:border-cyan-500/50 focus:outline-none font-mono text-sm"
          />
          <textarea
            placeholder="Description (optional)"
            value={newPlan.description} 
            onChange={e => setNewPlan({...newPlan, description: e.target.value})}
            className="w-full px-4 py-3 bg-[rgb(var(--bg-elevated))] border border-white/10 rounded-lg text-white placeholder-slate-500 focus:border-cyan-500/50 focus:outline-none font-mono text-sm resize-none"
            rows={2}
          />
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-3 bg-[rgb(var(--bg-elevated))] border border-white/10 rounded-lg text-white focus:border-cyan-500/50 focus:outline-none"
              />
            </div>
            <div>
              ...
              return (
                <button
                  ...
                  className={`aspect-square p-2 rounded-lg border transition-all relative ${
                    ...
                    isSelected ? 'border-cyan-500 bg-cyan-500/20 shadow-[0_0_10px_rgba(0,212,255,0.3)]' :
                    isToday ? 'border-purple-500 bg-purple-500/10' :
                    hasPlans ? 'border-white/10 bg-[rgb(var(--bg-elevated))] hover:border-cyan-500/30' :
        ...
        {(['all', 'pending', 'in_progress', 'completed', 'skipped'] as const).map(f => (
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-xs font-semibold font-mono transition-all ${
              filter === f 
                ? 'bg-gradient-to-r from-cyan-500 to-purple-500 text-white shadow-[0_0_10px_rgba(0,212,255,0.2)]' 
            ...
            return (
              <div 
                ...
                className={`group gc-card p-5 transition-all ${
                  plan.status === 'completed' ? 'opacity-60 hover:opacity-80' : 'hover:border-cyan-500/30'
                }`}
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-display font-bold text-white mb-1 group-hover:text-cyan-400 transition-colors">
                      {plan.title}
                    </h4>
                    {plan.description && (
