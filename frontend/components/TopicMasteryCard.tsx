...
export default function TopicMasteryCard({ topic }: TopicMasteryCardProps) {
  const getMasteryColor = (level: number) => {
    ...
    if (level >= 0.6) return 'bg-cyan-500';
  ...
  const getMasteryTextColor = (level: number) => {
    ...
    if (level >= 0.6) return 'text-cyan-400';
  ...
  return (
    <div className="gc-card p-4 border border-cyan-500/20 hover:border-cyan-500/40 transition-all">
      {/* Header */}
      ...
      <div className="mb-3">
        ...
        <div className="w-full bg-[rgb(var(--bg-surface))] rounded-full h-2 border border-cyan-500/10">
          <div
