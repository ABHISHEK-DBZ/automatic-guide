...
export default function BadgesDisplay() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats Overview */}
      {stats && (
        <div className="gc-card p-6 bg-gradient-to-br from-cyan-500/10 to-transparent border-cyan-500/30">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-slate-400 font-mono">Badges</div>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-slate-400 font-mono">Quizzes</div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-white">{stats.topics_mastered}</div>
              <div className="text-xs text-slate-400 font-mono">Mastered</div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-white">{stats.total_study_minutes}m</div>
              <div className="text-xs text-slate-400 font-mono">Study Time</div>
            </div>
          </div>
          {/* Progress bar */}
          <div className="mt-4">
            ...
            <div className="w-full bg-slate-700 rounded-full h-2">
              <div className="bg-gradient-to-r from-cyan-400 to-purple-400 h-2 rounded-full transition-all" style={{ width: `${progressPct}%` }}></div>
            </div>
          </div>
        </div>
      )}
          ...
          <button key={cat} onClick={() => setSelectedCategory(cat)}
            className={`px-3 py-1.5 text-xs rounded-lg capitalize transition-all font-mono ${
              selectedCategory === cat 
                ...
                : 'bg-slate-800/50 text-slate-400 hover:text-white hover:border-cyan-500/30 border border-white/10'
            }`}>
            {cat}
          </button>
        ))}
        ...
        {filteredBadges.map((badge) => (
          <div key={badge.name}
            className={`gc-card p-4 text-center transition-all duration-300 ${
              badge.earned
                ? 'border-cyan-500/30 bg-gradient-to-br from-cyan-500/10 to-transparent hover:border-cyan-500/50 hover:shadow-[0_0_15px_rgba(0,212,255,0.2)]'
