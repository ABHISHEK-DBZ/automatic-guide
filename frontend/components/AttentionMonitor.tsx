...
export default function AttentionMonitor() {
  ...
  const analyzeFrame = useCallback(async () => {
    ...
    try {
      ...
      // Draw video frame to canvas with reduced resolution
      const targetWidth = 320;  // Reduced from full resolution
      ...
      // Send to backend for analysis with shorter timeout
  ...
  const startWebcam = useCallback(async () => {
    try {
      ...
      // Start analyzing frames every 5 seconds to reduce backend load
      intervalRef.current = setInterval(analyzeFrame, 5000);
      
      // Do first analysis after a delay
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-300 font-mono text-sm mt-1">
                  Cannot connect to backend server. Please ensure the backend is running on http://localhost:8001
                </p>
              </div>
            </div>
          </div>
