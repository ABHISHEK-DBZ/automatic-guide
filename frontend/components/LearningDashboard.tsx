    ...
    <div className="h-full overflow-y-auto bg-[rgb(var(--bg-base))]">
      ...
      <div className="bg-gradient-to-r from-[rgb(var(--bg-elevated))] to-[rgb(var(--bg-surface))] border-b border-cyan-500/20 text-white p-4 sm:p-6">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-2xl sm:text-3xl font-display font-bold mb-2 text-white">Learning Analytics</h1>
          <p className="text-cyan-400 text-sm sm:text-base font-mono">Track your progress and mastery</p>
            ...
            {[7, 30, 90].map((days) => (
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-lg font-medium transition-colors text-sm sm:text-base ${
                  selectedPeriod === days
                    ? 'bg-cyan-500 text-black font-bold shadow-[0_0_10px_rgba(0,212,255,0.3)]'
                    : 'bg-white/10 hover:bg-cyan-500/20 text-cyan-400 border border-cyan-500/30'
                }`}
              >
                {days} days
              </button>
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        {recommendations && recommendations.study_tips.length > 0 && (
          <div className="gc-card p-6 border border-cyan-500/20">
          ...
          {quizHistory.length === 0 ? (
            ...
            <div className="space-y-3">
              {quizHistory.map((quiz) => (
                <div
                  key={quiz.session_id}
                  className="flex items-center justify-between p-4 border border-cyan-500/20 rounded-lg hover:bg-[rgb(var(--bg-elevated))]/50 transition-colors bg-[rgb(var(--bg-surface))]"
                >
